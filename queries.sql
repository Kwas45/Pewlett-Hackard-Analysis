...
-- Create another table for the data in salaries.csv
...
-- Create another table for the data in data_emp
...
-- Create another table for the data in title
...
-- Table name shortened to a nickname
SELECT ri.emp_no,
    ri.first_name,
	ri.last_name,
    de.to_date
...
LEFT JOIN dept_emp as de
ON ri.emp_no = de.emp_no
-- Finally, because this is a table of current employees, we need to add a filter, using the WHERE keyword
